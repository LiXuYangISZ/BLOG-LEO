---
title: 十三、垃圾回收器
date: 2024-04-05 15:45:13
tags: JVM
categorys: JVM从入门到入土
swiper_index: 1
description: JVM中所有的垃圾回收器介绍
---
...
![image-20210512144253383](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608908.png)
...
![image-20200713083443486](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608891.png)
...
![第17章_吞吐量与暂停时间](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608759.jpg)
...
![image-20200713093551365](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608936.png)
...
![image-20230304171940201](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608748.png)

### 13.2.3. 7 款经典收集器与垃圾分代之间的关系

![第17章_经典的7种GC](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608755.jpg)
...
![第17章_垃圾收集器组合](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608371.jpg)
...
![image-20230228150151322](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608417.png)
...
![image-20200713100703799](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608583.png)
...
![image-20200713102030127](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608652.png)
...
![image-20200713110359441](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608716.png)
...
- `-XX:MaxGCPauseMillis` 设置垃圾收集器最大停顿时间（即 STw 的时间）。单位是毫秒。
  ...
  - <mark>该参数使用需谨慎</mark>。[如果该参数设置的很小，就会导致堆被调整的很小，从而GC的频率就会很高，吞吐量会降低]
...
- `-XX:+UseAdaptivesizePolicy` 设置 Parallel Scavenge 收集器具有<mark>自适应调节策略</mark>
  ...
  > 无需记住所有的参数，只需要记住该垃圾回收器可以调整哪些参数就可以了，使用时从文档中查
...
![image-20200713205154007](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608893.png)
...
另外，由于在垃圾收集阶段用户线程没有中断，<mark>所以在 CMS 回收过程中，还应该确保应用程序用户线程有足够的内存可用</mark>。因此，CMS 收集器不能像其他收集器那样等到老年代几乎完全被填满了再进行收集，而是<mark>当堆内存使用率达到某一阈值时，便开始进行回收</mark>，以确保应用程序在 CMS 工作过程中依然有足够的空间支持应用程序运行。要是 CMS 运行期间预留的内存无法满足程序需要，就会出现一次“`Concurrent Mode Failure`” 失败，这时虚拟机将启动后备预案：临时==启用 Serial Old 收集器==来重新进行老年代的垃圾收集，这样停顿时间[STW]就很长了。
...
![image-20200713212230352](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608144.png)

**有人会觉得既然 Mark Sweep 会造成内存碎片，那么为什么不把算法换成 Mark Compact？**

答案其实很简单，因为当并发清除的时候，用 Compact 整理内存的话，原来的用户线程使用的内存还怎么用呢？==要保证用户线程能继续执行，前提的它运行的资源不受影响==嘛。Mark Compact 更适合“Stop the World” 这种场景下使用
...
- `-XX:CMSFullGCsBeforeCompaction` 设置在执行多少次 Full GC 后对内存空间进行压缩整理。【与上个参数配合使用】
...
- 如果你想要最小化地使用内存和并行开销，请选 Serial GC；老年代对应的Serial Old GC
- 如果你想要最大化应用程序的吞吐量，请选 Parallel GC；老年代对应着Parallel Old GC
- 如果你想要最小化 GC 的中断或停顿时间，请选 CMS GC；老年代对应着ParNew GC

### 13.6.5. JDK 后续版本中 CMS 的变化
...
![image-20200713215105293](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608161.png)

![image-20200713215133839](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608132.png)
...
2、最主要的应用是需要==低 GC 延迟，并具有大堆==的应用程序提供解决方案；如：在堆大小约 6GB 或更大时，可预测的暂停时间可以低于 0.5 秒；（G1 通过每次只清理一部分而不是全部的 Region 的增量式清理来保证每次 GC 停顿时间不会过长）。
...
4、HotSpot 垃圾收集器里，除了 G1 以外，其他的垃圾收集器使用内置的 JVM 线程执行 GC 的多线程操作【优先级比较低】，而 G1 GC 可以采用`应用线程`承担后台运行的 GC 工作，即当 JVM 的 GC 线程处理速度慢时，系统会调用应用程序线程帮助加速垃圾回收过程。
...
![image-20200713223244886](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608349.png)
...
<font color=red>每个 Region 都是通过指针碰撞来分配空间</font>

![image-20200713223509993](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608420.png)

### 13.7.7. G1 垃圾回收器的回收过程【了解】
...
> 如果需要，单线程、独占式、高强度的 Full GC 还是继续存在的。它针对 GC 的评估失败提供了一种失败保护机制，即强力回收。类似于Serial Old GC对CMS的一种保底方案一样

![第17章_G1-GC垃圾回收过程](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608401.jpg)



顺时针，Young gc -> Young gc + Concurrent mark->Mixed GC 顺序，进行垃圾回收。

应用程序分配内存，<mark>当年轻代的 Eden 区用尽时开始年轻代回收过程</mark>；G1 的年轻代收集阶段是一个<mark>并行的独占式</mark>【STW】收集器。在年轻代回收期，G1GC 暂停所有应用程序线程，启动多线程执行年轻代回收。然后<mark>从年轻代区间移动存活对象到 Survivor 区间或者老年区间，也有可能是两个区间都会涉及</mark>。
...
- 回收新生代也不得不同时扫描老年代？【因为老年代可能引用了新生代的某些对象】
- 这样的话会降低 MinorGC 的效率；
...
![image-20200713224716715](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608721.png)
...
![image-20200713225100632](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608890.png)

然后开始如下回收过程：

1. <mark>第一阶段，扫描根</mark>。根是指 static 变量指向的对象，正在执行的方法调用链条上的局部变量等。根引用连同 RSet 记录的外部引用作为扫描存活对象的入口。

   > 注意：年轻代是一定要有Rset的，年轻代的回收太频繁了。不适用Rset，每次都要判断老年代是否有引用指向新生代效率就太低了。反之，老年代回收不需要，因为老年代回收的时候，新生代也会进行回收
...
![image-20200713225810871](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608000.png)
...
| G1           | 并发、并行运行 | 作用于新生代、老年代 | 标记-压缩算法[整体]、复制算法[Regin] | 响应速度优先 | 面向服务端应用                           |

GC 发展阶段：Serial => Parallel（并行）=> CMS（并发）=> G1 => ZGC

### 13.8.2. 垃圾回收器组合

不同厂商、不同版本的虚拟机实现差距比较大。HotSpot 虚拟机在 JDK7/8 后所有收集器及组合如下图：（更新到了JDK14）

![第17章_垃圾收集器组合](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608955.jpg)
...
**<font color=orange>① -verbose:gc</font>**

这个只会显示总的 GC 堆的变化，如下：

```java
[GC (Allocation Failure) 80832K->19298K(228840K),0.0084018 secs]
...
**<font color=orange>② -verbose:gc -XX:+PrintGCDetails</font>**
...
**<font color=orange>③ -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimestamps -XX:+PrintGCDatestamps</font>**
...
> 如果想把 GC 日志存到文件的话，是下面的参数：`-Xloggc:/path/to/gc.log`
...
- <mark>Allocation Failure</mark> 表明本次引起 GC 的原因是因为在年轻代中没有足够的空间能够存储新的数据了。
...
### 13.9.2 Minor GC 日志图解

![image-20200714082555688](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608098.png)

### 13.9.3 Full GC 日志图解

![image-20210512194815354](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608198.png)



### 13.9.4 日志分析工具的使用

在生产开发中，我们也可以用一些工具去分析这些 GC 日志

常用的日志分析工具有：[GCViewer](https://sourceforge.net/projects/gcviewer/)、[GCEasy](https://gceasy.io/)、GCHisto、GCLogViewer、Hpjmeter、garbagecat 等

只需要在JVM参数中加入`  -Xloggc:./logs/gc.log`，控制台的一些日志就会输出到日志中

- GCViewer界面

![image-20230304160528881](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608509.png)

- GCEasy：**<font color=red>☆☆☆☆☆</font>**

![image-20230304162020051](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608553.png)![image-20230304162121922](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608592.png)
...
![image-20210512195426194](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608962.png)

现在 G1 回收器已成为默认回收器好几年了。

我们还看到了引入了两个新的收集器：ZGC（JDK11 出现）和 Shenandoah（Open JDK12）。主打特点：低停顿时间

![image-20210512195528695](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608898.png)
...
![image-20200714090608807](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608261.png)
...
![image-20210512200236647](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608314.png)
...
![image-20200714091201073](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608346.png)

![image-20200714091401511](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608374.png)
...
![image-20200714093243028](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608800.png)
...
![image-20200714093604012](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202303061608892.png)

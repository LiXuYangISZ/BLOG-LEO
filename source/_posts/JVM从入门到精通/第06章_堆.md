...
![image-20220329143619498](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291436968.png)
...
![Snipaste_2022-03-28_18-17-59](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291440954.jpg)

![Snipaste_2022-03-28_18-17-15](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291440947.jpg)

- 《Java 虚拟机规范》规定，堆可以处于==物理上不连续==的内存空间中，但在==逻辑上==它应该被视为==连续==的。

- 所有的线程共享 Java 堆，在这里还可以划分==线程私有的缓冲区==（Thread Local Allocation Buffer，TLAB）(可以更好的解决并发)。
...
- 《Java 虚拟机规范》中对 Java 堆的描述是：==所有的对象实例以及数组都应当在运行时分配在堆上==。（`The heap is the run-time data area from which memory for all class instances and arrays is allocated`）
  - 从实际角度看:**<font color=red>"几乎"所有的对象实例都在这里分配内存</font>**(出现逃逸分析,标量分配 会在栈上分配)
...
![image-20200706201904057](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291509428.png)
...
![image-20200706203835403](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291851669.png)

![Meta](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291852750.png)
...
![Snipaste_2022-03-29_16-16-22](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291900582.jpg)

![Snipaste_2022-03-29_16-16-49](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291901538.jpg)



### 6.1.2. 堆空间内部结构（JDK7）

![第08章_堆空间-java7](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291606983.jpg)

### 6.1.3. 堆空间内部结构（JDK8）

![第08章_堆空间-java8](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203291606152.jpg)
...
![JVM当下最大堆内存](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203292222800.jpg)

- 手动设置：-Xms600m -Xmx600m,运行之. 我们出现了疑惑,后面进行解密!哈哈哈.

![image-20220329222554212](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203292225470.png)

- 打开try...catch...,进行查看分析.

![命令行查看进程信息](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203292246803.png)

- 使用PrintGCDetails,查看堆内存的详细信息.

![image-20220329224911087](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203292249351.png)
...
![image-20220330121733575](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203301217015.png)

![image-20220330121951858](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203301219267.png)
...
![第08章_堆空间细节](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203311433796.jpg)
...
在 HotSpot 中，Eden 空间和另外两个 survivor 空间缺省所占的比例是 8：1：1 (因为有个自适应的内存分配原则,**实际测试<font color=red>6:1:1</font>**)
...
![image-20210510105849497](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203311437944.png)
...
![image-20220331144815281](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203311448669.png)
...
![第08章_新生代对象分配与回收过程](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203311539655.jpg)
...
![image-20200707091058346](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203311549048.png)
...
![image-20220331160620414](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203311606998.png)
...
**备注:**当Survivor满了,超出部分直接放在老年区,其他部分保持不变.等下次Eden满了,触发Minor GC,再对Survivor进行垃圾清理. 即 **<font color=red>Survivor区的垃圾回收是被动的</font>**

**threshold:阈值**

![image-20200707095606813](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203312157369.png)
...
5. 由 Eden 区、survivor space0（From Space）区向 survivor space1（To Space）区复制时，对象大小大于 To Space 可用内存，则把该对象转存到老年代，且老年代的可用内存小于该对象大小，则会触发。
...
![三种GC对比](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204011815662.png)
...
![image-20200707101511025](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204011822133.png)
...
![image-20200707101543871](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204011822653.png)
...
![image-20220401185049540](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204011850647.png)
...
![第08章_TLAB](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204011906187.jpg)
...
![第08章_对象分配过程](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202203311434924.jpg)
...
在 JDK6 Update24 之后(jdk7)，HandlePromotionFailure 参数不会再影响到虚拟机的空间分配担保策略，观察 openJDK 中的源码变化，虽然源码中还定义了 HandlePromotionFailure 参数，但是在代码中已经不会再使用它(该参数已经毫无意义了)。JDK6 Update 24 之后的规则变为<mark>只要老年代的连续空间大于新生代对象总大小或者历次晋升的平均大小就会进行 Minor GC</mark>，否则将进行 Full GC。
...
在 Java 虚拟机中，对象是在 Java 堆中分配内存的，这是一个普遍的常识。但是，有一种特殊情况，那就是<mark>如果经过逃逸分析（Escape Analysis）后发现，一个对象并没有逃逸出方法的话，那么就可能被优化成栈上分配</mark>.。这样就无需在堆上分配内存，也无须进行垃圾回收了。这也是最常见的==堆外存储技术==。
...
public class EscapeAnalysis {
    ...
    public void useEscapeAnalysis1(){
        //基本数据类型和堆没关系，在栈帧中的局部变量表中。
		//这里进行逃逸分析，是为了将本来分配在堆上的对象，分配到栈帧中。
...
三、<mark>分离对象或标量替换</mark>：有的对象可能不需要作为一个连续的内存结构存在也可以被访问到，那么对象的部分（或全部）可以不存储在内存，而是存储在 CPU 寄存器(栈)中。

#### 栈上分配

JIT 编译器在编译期间根据逃逸分析的结果，发现==如果一个对象并没有逃逸出方法的话，就可能被优化成栈上分配==。分配完成后，继续在调用栈内执行，最后线程结束，栈空间被回收，局部变量对象也被回收。这样就无须进行垃圾回收了。
...
![逃逸分析](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204012239235.png)

![image-20220401223640281](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204012236617.png)

![image-20220401223802722](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204012238234.png)
...
![image-20220401230925542](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204012309577.png)**

![image-20220401231123059](https://blog-photos-lxy.oss-cn-hangzhou.aliyuncs.com/img/202204012311576.png)
